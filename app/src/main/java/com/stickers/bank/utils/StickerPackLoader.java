...
import static com.stickers.bank.provider.StickerContentProviderApi.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.stickers.bank.provider.StickerContentProviderApi.PRIVACY_POLICY_WEBSITE;
import static com.stickers.bank.provider.StickerContentProviderApi.PUBLISHER_EMAIL;
import static com.stickers.bank.provider.StickerContentProviderApi.PUBLISHER_WEBSITE;
...
import com.stickers.bank.data.model.Sticker;
import com.stickers.bank.data.model.StickerPack;
import com.stickers.bank.provider.StickerContentProviderApi;
...
public class StickerPackLoader {
    ...
    public static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        final Cursor cursor = context.getContentResolver().query(StickerContentProviderApi.AUTHORITY_URI, null, null, null, null);
        if (cursor == null) {
            throw new IllegalStateException("could not fetch from content provider, " + BuildConfig.CONTENT_PROVIDER_AUTHORITY_API);
        }
    ...
    private static ArrayList<StickerPack> fetchFromContentProvider(Cursor cursor) {
        ...
        cursor.moveToFirst();
        do {
            final String identifier = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.STICKER_PACK_IDENTIFIER_IN_QUERY));
            final String name = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.STICKER_PACK_NAME_IN_QUERY));
            final String publisher = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.STICKER_PACK_PUBLISHER_IN_QUERY));
            final String trayImage = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.STICKER_PACK_ICON_IN_QUERY));
            final String androidPlayStoreLink = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY));
            ...
            final String licenseAgreementWebsite = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.LICENSE_AGREENMENT_WEBSITE));
            final String imageDataVersion = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.IMAGE_DATA_VERSION));
            final boolean avoidCache = cursor.getShort(cursor.getColumnIndexOrThrow(StickerContentProviderApi.AVOID_CACHE)) > 0;
            final boolean animatedStickerPack = cursor.getShort(cursor.getColumnIndexOrThrow(StickerContentProviderApi.ANIMATED_STICKER_PACK)) > 0;
    ...
    private static List<Sticker> fetchFromContentProviderForStickers(String identifier, ContentResolver contentResolver) {
        ...
        final String[] projection = {StickerContentProviderApi.STICKER_FILE_NAME_IN_QUERY, StickerContentProviderApi.STICKER_FILE_EMOJI_IN_QUERY};
        final Cursor cursor = contentResolver.query(uri, projection, null, null, null);
        List<Sticker> stickers = new ArrayList<>();
        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            do {
                final String name = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.STICKER_FILE_NAME_IN_QUERY));
                final String emojisConcatenated = cursor.getString(cursor.getColumnIndexOrThrow(StickerContentProviderApi.STICKER_FILE_EMOJI_IN_QUERY));
    ...
    private static Uri getStickerListUri(String identifier) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY_API).appendPath(StickerContentProviderApi.STICKERS).appendPath(identifier).build();
    }

    static Uri getStickerAssetUri(String identifier, String stickerName) {
        return new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY_API).appendPath(StickerContentProviderApi.STICKERS_ASSET).appendPath(identifier).appendPath(stickerName).build();
    }
}
